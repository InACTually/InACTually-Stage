...
export default class KinectRoomNode extends RoomNodeBase implements CaptureDevice {
	...
	constructor(publisher: IRoomNodePublisher, onLoadCb: { (roomNode: RoomNodeBase): void }, uid?: string, position?: THREE.Vector3, orientation?: THREE.Quaternion) {
		super("kinect", publisher, onLoadCb, uid, position, orientation);
		this.setup(onLoadCb);

	}

	public override setup(onLoadCb: { (roomNode: RoomNodeBase): void }): void {
		this.m_caption.value = "Kinect"
		this.m_roomNodeType = RoomNodeType.RNT_KINECT;
